    ...
    def run(self, pdf_path: Path, spec: BookSpec) -> list[CheckResult]:
        ...
        if warn_low_images:
            results.append(CheckResult(
                ...
                message=f"{len(warn_low_images)} image(s) below {RESOLUTION_WARN_LOW}ppi recommended",
        ...
        if warn_high_images:
            results.append(CheckResult(
                check_name=self.name,
                status=CheckStatus.WARN,
                message=f"{len(warn_high_images)} image(s) above {RESOLUTION_WARN_HIGH}ppi (increases file size)",
                severity=Severity.WARNING,
        ...
        if not results:
            min_overall = min(
                min(img.effective_dpi_x, img.effective_dpi_y) for img in images
            )
            max_overall = max(
                max(img.effective_dpi_x, img.effective_dpi_y) for img in images
            )
            results.append(CheckResult(
                check_name=self.name,
                status=CheckStatus.PASS,
